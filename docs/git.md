...
- 릴리스마다 **주석 태그(annotated tag)** `vX.Y.Z`를 만들고 푸시한다 — `CHANGELOG.md`의 버전과 동기.
- 형식: `git tag -a v0.1.0 -m "v0.1.0 — 한 줄 요약"` → `git push origin v0.1.0`
- 버전 규칙(SemVer): 큰 방향 전환 = major, 게임/기능 추가 = minor, 버그 수정·다듬기 = patch.
- 현재: `v0.1.0` (2026-07-16, 첫 공개)
