...
`dori` — **결정 돕기 · 랜덤 뽑기 · 복불복**을 모두 포괄하는 Phaser 3 기반 **도리도리 잼잼(결정 도우미) 모음 사이트**(놀이처럼 즐거운 연출·어포던스). 빌드 도구 없이 GitHub Pages로 배포됩니다.
사용자 노출 문면의 총칭은 **'도리도리 잼잼'**이며, 대외 카피에서 **'게임'·'플레이' 용어는 쓰지 않는다**(내부 코드·씬 키·docs 용어는 기존 정책대로 유지). 오락 목적 고지 문구는 '모든 도구는 오락 목적'으로 통일.
