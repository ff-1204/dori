...
import { C, css, FONT, PLAYER, EASE, RADIUS, LAYOUT } from '../theme.js';
...
export default class DrawScene extends MiniGame {
  ...
  onCreate() {
    ...
    // 공통 레이아웃 그리드(LAYOUT): 헤더48 / 태그라인128 / 문구190 / 풀 칩250– / 상자(y700) / 링크1002(±150) / 주 버튼1104
    makeHeader(this, '뽑기 상자', '뽑힌 건 빠지고, 남은 건 보여요');
    ...
    // 안내·결과 공용 문구 — '(뽑힌 건 제외)'는 풀 칩이 시각적으로 보여주므로 문구에선 생략(32px 폭 확보)
    this.hint = this.add.text(this.cx, LAYOUT.msgY, '뽑기를 누르면 하나가 나와요', {
    ...
    this.editBtn = makeSubLink(this, this.cx - LAYOUT.linkDX, LAYOUT.linksY, '✎ 편집', () => this.openEditor());
    this.resetBtn = makeSubLink(this, this.cx + LAYOUT.linkDX, LAYOUT.linksY, '↺ 전부 되돌리기', () => this.resetDrawn(), C.warning);
  }
  ...
  renderPool() {
    ...
    const startY = 250; // 문구(y190) 아래 게임판 밴드 시작
  ...
  buildBox() {
    // 상자(y 610–790, 뚜껑 590) — 카드가 여기서 튀어나온다. 판 밴드 광학 중심에 맞춰 배치
    this.box = this.add.container(this.cx, 700);
    const g = this.add.graphics();
  ...
  reveal(idx) {
    ...
    this.card = this.add.container(this.cx, 680).setDepth(20);
    const g = this.add.graphics();
    ...
    this.tweens.add({
      targets: this.card, y: 500, scale: 1, alpha: 1, duration: 420, ease: EASE.popIn,
    });

    this.burst(this.cx, 610, color, 30);
  ...
  renderEditorChips() {
    ...
    this.items.forEach((name, i) => chip(`${name}  ✕`, { textColor: this.itemColor(i) }, () => this.removeItem(i)));
    if (this.items.length < MAX_I) chip('+ 추가', { outline: true }, () => this.addItem());
